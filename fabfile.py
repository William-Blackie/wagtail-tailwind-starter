...
def build(c):
    ...
    local("docker-compose build web utils")
...
def import_data(c, database_filename):
    ...
    print("Stopping local containers")
    ...
    dexec(
        "pg_restore --clean --no-acl --if-exists --no-owner --host db \
            --username={project_name} -d {database_name} /app/database_dumps/{database_filename}".format(
...
def push_staging_media(c):
    """Push local media content to staging instance"""
...
def pull_database_from_heroku(c, app_instance):
    ...
    dexec(
        "heroku pg:backups:download --output={database_dumps}/{datestamp}.dump --app {app}".format(
            database_dumps=LOCAL_DATABASE_DUMPS_FOLDER,
            app=app_instance,
            datestamp=datestamp,
        ),
        service="utils",
    ...
    dexec(
        "rm {database_dumps}/{datestamp}.dump".format(
            database_dumps=LOCAL_DATABASE_DUMPS_FOLDER, datestamp=datestamp
        ),
        service="utils",
    )
